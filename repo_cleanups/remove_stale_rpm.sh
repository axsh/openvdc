...
stale_weeks=3               ## Any git branches which have not been pushed
                            ...
                            ## "stale".
#rpm_repo_dir=/home/chris/Misc/var/www/html/openvdc-repos
...
#for bch in $(git ls-remote --heads https://github.com/axsh/openvdc/ | awk '{print $2}'); do
#    echo $bch
#done

## git for-each-ref --sort=-committerdate refs/remotes --format='%(HEAD)%(color:yellow)%(refname:short)|%(color:bold green)%(committerdate:relative)|%(color:blue)%(subject)|%(color:magenta)%(authorname)%(color:reset)'|column -ts'|'


### git for-each-ref --sort=-committerdate refs/remotes --format='%(color:yellow)%(refname:short)|%(color:bold green)%(committerdate:relative)|%(color:blue)%(subject)%(color:reset)'|column -ts'|'
