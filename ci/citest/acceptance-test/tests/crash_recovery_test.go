...
func TestCrashRecovery(t *testing.T) {
	...
	//Simulate crash
	RunSshWithTimeoutAndReportFail(t, executor_lxc_ip, "sudo systemctl stop mesos-agent", 10, 5)

	//Give scheduler some time to register crash
	time.Sleep(3 * time.Minute)

	RunSshWithTimeoutAndReportFail(t, executor_lxc_ip, "sudo systemctl start mesos-agent", 10, 5)
