...
def stage_integration(label) {
  node(label) {
    ...
    sh "ssh yumrepo@192.168.56.111 /bin/bash -c 'cd /data/openvdc-integration/deployment/integration/ ; pwd'"
    sh "ssh yumrepo@192.168.56.111  /bin/bash -c 'cd /data/openvdc-integration/deployment/integration/ ; ./build.sh'"
  }
}
