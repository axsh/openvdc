...
def stage_integration(label) {
  node(label) {
    ...
    write_build_env(label)
    sh "cp /var/lib/jenkins/.ssh/id_rsa.pub deployment/integration "
    ...
    sh "tar cf - . |  ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no vagrant@192.168.56.61 tar xf - -C /var/tmp --warning=no-timestamp  --no-overwrite-dir" 
